    ...
    def tree_to_tags(cls, root: Node) -> []:
        ...
        while len(stack) != 1 or stack[0].label != root.label:
            ...
            if not node.is_right_child() and node.parent.right is not None:
                ...
                logging.debug("<== \t REDUCE[ {0} --> {1} ]".format(*(node.label, node.parent.label)))
        ...
        for tag in tags:
            if tag == TetraType.r:  # shift
                ...
                if tag == TetraType.R:  # normal reduce
                    ...
                    created_node_stack.append(cls._unary_reduce(node, created_node_stack.pop()))
        if len(input_seq) != 0:
            raise ValueError("All the input sequence is not used")
        return node

    @classmethod
    def tree_to_original_tags(cls, root):
        """ convert right-corner transformed tree to original tetratags """
        actions = []
        lc = LeftCornerTransformer.extract_left_corner_no_eps(root)
        ...
        while len(stack) != 1 or stack[0].label != root.label:
            ...
            if node != node.parent.right and node.parent.right is not None:
                ...
                if node.parent.node_info.type == NodeType.NT:
                    actions.pop()
                ...
                stack.append(node.parent)
                    ...
                    stack) == 1 and node.node_info.type == NodeType.PT or node.node_info.type == NodeType.NT:
