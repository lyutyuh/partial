...
def dep2lex(dep_tree, language="English"):
    ...
    return dfs(
        dep_tree.nodes[0]["deps"]["root"][0] if "root" in dep_tree.nodes[0]["deps"] else dep_tree.nodes[0]["deps"]["ROOT"][0]
    )
...
if __name__ == "__main__":
    ...
    for language in [
        ...
        for path, split in zip(paths, ["train", "dev", "test"]):
            ...
            with open(os.path.dirname(repo_directory)+f"/bht/{language}.bht.{split}", "w") as fout:
