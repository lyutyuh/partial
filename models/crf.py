...
class CRF(nn.Module):
    ...
    def __init__(
            self, nb_labels, max_depth, bos_tag_id, eos_tag_id, pad_tag_id=None,
            batch_first=True
    ):
        super().__init__()
        ...
        self.state_size = nb_labels * max_depth
        self.BOS_TAG_ID = bos_tag_id
        self.EOS_TAG_ID = eos_tag_id
        self.PAD_TAG_ID = pad_tag_id
        self.batch_first = batch_first

        self.transitions = nn.Parameter(torch.empty(self.state_size, self.state_size))
        self.init_weights()

    def _get_index_range_by_depth(self, d):
        first_idx = (d - 1) * self.nb_labels + 1
        last_idx = d * self.nb_labels + 1
    ...
    def init_weights(self):
        ...
        self.transitions.data[self.BOS_TAG_ID, self.nb_labels + 1:] = MINUS_INF
        ...
        for d in range(self.max_depth + 1):
            ...
            self.transitions.data[first_idx:last_idx + 1, first_idx:last_idx + 1] = MINUS_INF
            if d > 1:
                d_m1_first_idx, _ = self._get_index_range_by_depth(d - 1)
                self.transitions.data[first_idx:last_idx + 1, :d_m1_first_idx] = MINUS_INF
            if d < self.max_depth:
                _, d_p1_last_idx = self._get_index_range_by_depth(d + 1)
                self.transitions.data[first_idx:last_idx + 1, d_p1_last_idx + 1:] = MINUS_INF
